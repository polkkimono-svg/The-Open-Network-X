...
3. **46 opcodes across seven byte ranges** (`docs/specification/tvm-instruction-set.md` §4): stack manipulation, arithmetic, conversion, bit/byte-strings, cell/value access, cryptographic primitives, and control flow — sized to cover every `execution.md` §3.3 requirement plus the minimum control flow needed for execution to be possible at all, explicitly not sized to match or exceed any existing VM's opcode count.
