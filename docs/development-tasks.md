...
- **Subsystem:** Storage / State Model (`crates/onx-state-model`)
...
- **Subsystem:** Execution / VM (`crates/onx-execution`)
...
- **Deliverables:**
  - Extended instruction handlers in `crates/onx-execution/src/interpreter.rs`.
  - Expanded opcode test suite in `crates/onx-execution/tests/execution_tests.rs`.
- **Acceptance Criteria:**
...
- **Deliverables:**
  - `crates/onx-execution/src/dictionary.rs` and `crates/onx-execution/src/continuation.rs`.
...
- **Subsystem:** Networking (`crates/onx-networking`)
...
- **Subsystem:** Consensus (`crates/onx-consensus`)
...
- **Subsystem:** Blockchain / Sync (`crates/onx-blocks`)
...
- **Subsystem:** Sharding (`crates/onx-sharding`)
...
- **Subsystem:** Transactions / Messaging (`crates/onx-transactions`)
...
- **Subsystem:** Validator Ops / Economics (`crates/onx-consensus`, `crates/onx-economics`)
...
- **Deliverables:**
  - `crates/onx-consensus/src/election.rs` and `crates/onx-economics/src/slashing.rs`.
...
- **Deliverables:**
  - `crates/onxd/` binary crate with `main.rs`, configuration parsing, and system service hooks.
...
- **Subsystem:** Developer Tooling / RPC (`crates/onx-rpc`)
...
- **Deliverables:**
  - `crates/onx-rpc/` crate exposing HTTP/WebSocket services.
...
- **Subsystem:** Developer Tools (`crates/onx-cli`)
...
- **Deliverables:**
  - `contracts/system/elector.tvm`, `contracts/system/config.tvm` and assembly source code.
...
- **Subsystem:** Off-Chain / Channels (`crates/onx-payment-channels`)
...
- **Subsystem:** Telemetry / Node Software (`crates/onxd`)
...
- **Deliverables:**
  - `crates/onx-telemetry/` crate and Grafana dashboard templates (`docs/telemetry/grafana.json`).
- **Acceptance Criteria:**
...
- **Subsystem:** Deployment / Network Launch (`crates/onx-genesis`)
...
- **Deliverables:**
  - `crates/onx-genesis/` binary crate.
  - Testnet initialization documentation (`docs/launch_guide.md`).
- **Acceptance Criteria:**
...
2. [ ] **Research Logbook:** An entry has been added to `docs/RESEARCH_LOGBOOK.md` containing `[ANSWER]` and `[QUESTION]`.
