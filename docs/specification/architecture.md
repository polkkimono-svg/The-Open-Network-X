...
| ONX-ARCH-009 | What are the payment-channel and payment-channel-network (lightning-style) protocol rules, and what Execution/VM primitives (e.g. Merkle-proof operations over an embedded virtual blockchain) do they require? | Partially resolved: `docs/specification/execution.md` §3.5 and ADR-0007 reserve the required Merkle-proof/pruned-branch VM primitive. The payment-channel protocol rules themselves remain open, in a future Payment-channels specification. |
